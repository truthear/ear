...
#define FINAL_BUFF_SIZE (PCM_OUT_SIZE*1000)

volatile short final_buff[FINAL_BUFF_SIZE];
volatile int final_buff_idx = 0;
const int final_buff_max = FINAL_BUFF_SIZE;
...
void DMA1_Stream3_IRQHandler(void)
{
  ...
  if (DMA_GetFlagStatus(DMA1_Stream3, DMA_FLAG_TCIF3) != RESET)
  {
    ...
    for ( int n = 0; n < PCM_OUT_SIZE; n++ )
        {
          int t = final_buff_idx;
          final_buff[t] = (short)decode_buf[n];
          t++;
          if ( t == final_buff_max )
            t = 0;
          final_buff_idx = t;
